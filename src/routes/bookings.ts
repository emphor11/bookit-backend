...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (err.message === "NOT_ENOUGH_SEATS") return res.status(409).json({ error: "NOT_ENOUGH_SEATS" });
    if (err.message === "SLOT_NOT_FOUND") return res.status(404).json({ error: "SLOT_NOT_FOUND" });
    console.error(err);
